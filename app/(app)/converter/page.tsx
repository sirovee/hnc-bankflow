                    ...
                    return(
                      <tr key={t.id} className={`border-b border-border hover:bg-slate-50 transition-colors group ${rc}`}>
                        ...
                        <td className="px-3 py-2 max-w-[220px]">
                          <input key={`desc-${t.id}-${t.description}`} className="cell-input w-full" defaultValue={t.description} onKeyDown={e=>{if(e.key==='Enter')(e.target as HTMLInputElement).blur()}} onBlur={e=>{if(e.target.value!==t.description)tx.editCell(ri,'description',e.target.value)}}/>
