...
export function useTransactions(initial: EnrichedTransaction[]=[]) {
  ...
  const enriched=useMemo<EnrichedTransaction[]>(()=>{
    ...
    return rows.map((t,i)=>({...t,_val:vals[i]??null,_isDuplicate:dups.has(i),_isRedFlag:RED.test(t.description||''),_isVerified:verified.has(i),_parseWarn:vals[i]?.parseWarn??false,_selected:!!selected[t.id]}))
  ...
  const addRow=useCallback((idx?:number)=>{
    ...
    const blank:EnrichedTransaction={id,date:'',txtype:'',description:'',paidin:'',paidout:'',balance:'',_confidence:null,_page:null,_ocr:{},category:null,_match:emptyMatch(),_auto_corrected:false,_suggested:false,_suggestion:null,_isDuplicate:false,_isRedFlag:false,_isVerified:false,_parseWarn:false,_selected:false,_val:null}
  ...
  return{rows:enriched,audit,recon,stats,selectedCount:selectedIndices.length,selectedIndices,editCell,addRow,deleteRow,duplicateRow,moveAmount,restoreEdit,loadTransactions,bulkDelete,bulkVerify,bulkUnverify,toggleSelect,toggleSelectAll}
}
